...
function getCellDescription_(cellAddress) {
  ...
  var cellDescriptions = {
    ...
    'AP3': '想定関税閾値。通常は500に設定。ゲームなど送料上限があるものは必ず切り替え（例：ゲームの場合は20）。',
...
function assistantGetFaqList_() {
  return {
    ...
    data: [
      ...
      {
        category: '翻訳・AI',
        question: '翻訳が動かない・反応しない',
        answer: '【確認手順】\n' +
          '1. APIキーが設定されているか確認\n' +
          '   → メニュー → 初期設定 → API設定タブ\n\n' +
          '2. シートを更新（F5キーまたはブラウザ更新）\n' +
          '   → 設定変更後は更新が必要です\n\n' +
          '3. APIキーを再入力\n' +
          '   → コピペミスがないか確認\n' +
          '   → 前後にスペースが入っていないか確認\n\n' +
          '4. 翻訳対象の行が正しく選択されているか確認'
      },
      {
        category: '翻訳・AI',
        question: 'APIエラーが出る',
        answer: '【よくある原因と対処】\n\n' +
          '■ 401エラー（認証エラー）\n' +
          '→ APIキーが間違っています。再度コピーして貼り付けてください。\n\n' +
          '■ 429エラー（レート制限）\n' +
          '→ 短時間に大量のリクエストを送りました。数分待ってから再実行してください。\n\n' +
          '■ 残高不足エラー\n' +
          '→ OpenAI/Anthropicの管理画面でクレジット残高を確認してください。\n\n' +
          '■ ネットワークエラー\n' +
          '→ インターネット接続を確認し、再度実行してください。'
      },
      {
        category: '翻訳・AI',
        question: 'AIのAPIキーとEAGLEのAPIトークンの違いは？',
        answer: '【重要】この2つは全く別物です！\n\n' +
          '■ AIのAPIキー（OpenAI / Claude）\n' +
          '→ 翻訳機能で使用\n' +
          '→ 設定場所：初期設定 → API設定タブ\n' +
          '→ 形式例：sk-xxxxx... または sk-ant-xxxxx...\n\n' +
          '■ EAGLEのAPIトークン\n' +
          '→ EAGLE連携で使用（テンプレート取得など）\n' +
          '→ 設定場所：初期設定 → EAGLE連携タブ\n' +
          '→ EAGLEの管理画面から取得\n\n' +
          '【よくあるミス】\n' +
          '・EAGLEのトークンをAI設定に入れてしまう\n' +
          '・AIのキーをEAGLE設定に入れてしまう'
      },
      // ========== UI・表示関連 ==========
      {
        category: 'UI・表示',
        question: 'サイドパネルが開かない',
        answer: '【対処手順】\n\n' +
          '1. ブラウザをリロード（F5キー）\n\n' +
          '2. ブラウザのキャッシュをクリア\n' +
      ...
      {
        category: 'UI・表示',
        question: 'メニューが表示されない',
        answer: '【対処手順】\n\n' +
          '1. ページを更新（F5キー）\n\n' +
          '2. 数秒待つ\n' +
          '   → メニューの読み込みに時間がかかる場合があります\n\n' +
          '3. スプレッドシートを閉じて再度開く\n\n' +
          '4. ライブラリの権限を再承認\n' +
          '   → メニュー → 拡張機能 → Apps Script → 実行'
      },
      // ========== 価格計算関連 ==========
      {
        category: '価格計算',
        question: '価格計算が合わない・おかしい',
        answer: '【チェックポイント】\n\n' +
          '■ 設定セルを確認（1〜3行目）\n' +
          '・F1：手数料率\n' +
          '・F2：広告費率\n' +
      ...
      },
      {
        ...
        answer: '【チェックポイント】\n\n' +
          ...
          '→ ゲーム等は20に設定が必要\n\n' +
          '■ AD列（想定関税）の確認\n' +
          '→ この値がシッピングポリシーの判定に使われます'
      },
      // ========== EAGLE連携関連 ==========
      {
        category: 'EAGLE連携',
        question: 'テンプレートが判定されない',
        answer: '【チェックポイント】\n\n' +
          '1. EAGLEからテンプレートを取得済みか\n' +
          '   → メニュー → EAGLE連携 → テンプレート取得\n\n' +
          '2. O2セルの設定を確認\n' +
          '   → テンプレート名称が正しく設定されているか\n' +
          '   → ゲーム/カードなどは専用テンプレートに切替\n\n' +
          '3. 必須列の入力を確認\n' +
          '   → X列：配送方法\n' +
          '   → AE列：状態（新品/中古）\n\n' +
          '4. Import_Templatesシートを確認\n' +
          '   → EAGLEのテンプレートが正しく取得されているか'
      },
      {
        category: 'EAGLE連携',
        question: 'シッピングポリシーが出ない',
        answer: '【チェックポイント】\n\n' +
          '■ 3つの必須要素を確認\n' +
          '1. AD列：想定関税額\n' +
          '2. X列：配送方法\n' +
          '3. AE列：状態（新品/中古）\n' +
          '→ この3つが全て入力されていないと判定できません\n\n' +
          '■ EAGLEからポリシーを取得済みか\n' +
          '→ メニュー → EAGLE連携 → ポリシー取得\n\n' +
          '■ Import_Policiesシートを確認\n' +
          '→ ポリシーが正しく取得されているか'
      },
      {
        category: 'EAGLE連携',
        question: 'EAGLE連携でエラーが出る',
        answer: '【よくある原因と対処】\n\n' +
          '■ APIトークンのミス\n' +
          '→ EAGLEの管理画面から再度コピー\n' +
          '→ 初期設定 → EAGLE連携タブで再入力\n' +
