...
function getRelistingTranslationSkillContent() {
  ...
  return historyBlock.concat([
    ...
    '| `v5インポート` | 新方式。既にv5へ入っているデータを使い、M列Titleは全件組み替え、Condition/Item Specificsは検査・補正する |',
    '',
    ...
    '### `v5インポート` 上で実行する場合',
    '',
    '- モードA = 既存v5主軸: 既に入っているJ/M列Title、N列Condition、P:BCのItem Specificsを主軸にする。ただしM列Titleは対象全行で必ず組み替え、既存M列・J列と同一にしない。BF列URLは必要時だけ補助として使う。',
    '- モードB = URL参照つき精査: BF列URLの仕入れ先ページを補助情報として参照し、既存v5内容と矛盾があれば修正する。M列TitleはモードAと同じく対象全行で必ず組み替える。ページが見られない場合はモードA相当で処理する。',
    '',
    '`v5インポート` モードでは、Titleだけは全件組み替えが必須。Condition/IS/category/tagは原則として既存データの検査・補正を行い、必要な場合だけ修正する。ユーザーが「作り直し」と明示した行だけ、Condition/ISも再生成してよい。',
    '',
    '## 入力: 旧方式 `インポート用`',
    '',
    '| 列 | 内容 | 補足 |',
    ...
    '## Title（M列）',
    '',
    '旧方式・新方式ともに、M列Titleは対象全行で必ず組み替える。前回Title・取り込み時Title・J列元タイトル・既存M列Titleと同一は禁止。',
    '',
    'なぜ組み替えるか: 前回そのTitleで売れなかった。同じTitleで再出品しても、eBay検索（Cassini）は同一リスティングとみなし、再び埋もれる。語順を変え同義語に置き換えることで検索面を変え、新規リスティングとして拾われやすくする。',
    '',
    'Title組み替えルール:',
    '',
    '- 既存M列TitleとJ列元タイトルを情報源として使ってよいが、そのままコピーしない。',
    '- 商品の事実は変えない。確認できないブランド・型番・数量・素材・状態は追加しない。',
    '- 既存Titleが80字以内で禁止語なしでも、全件で別Titleにする。',
    '',
    'SEO語順で再構成する。次の順序を基本にし、検索向けに並べ替える。',
    '',
    '1. ブランド（先頭）',
    '2. 素材・スペック（Gold Plated / Silk / SS / 18K 等）',
    '3. 主要特徴・コレクション',
    '4. 商品タイプ（Brooch / Ring / Necklace / Card / Tie 等）',
    '5. サイズ・採寸',
    '6. その他（Vintage / 96P / France 等）',
    '',
    'ブランドは先頭25〜30字以内に置く。',
    '',
    '同義語置換を最低1語行う（検索面を変えるため）。例: Brooch→Pin / Necklace→Chain / Ring→Band / Bow Tie→Bowtie / Earrings→Studs / Gold Plated→GP / Sterling Silver→925 Silver / Holo→Holographic / No Box→NB。商品に正確に当てはまる同義語のみ使う。誤った言い換えは禁止。',
    '',
    '語は元のTitle、またはCondition・説明にある語だけを使う。新情報・新語の捏造は禁止。数値・型番はそのまま転記する。',
    '',
    '書込前に、対象全行で次をコード等で機械検査する。1件でも該当すれば書き込まず修正する。',
    '',
    '- 「新M列Title != 旧M列Title」',
    '- J列がある場合は「新M列Title != J列」',
    '- 同義語置換が1語以上入っている',
    '- 語順が元と大きく異なる',
    '',
    'タイトルだけを全件組み替え、Condition/ISは必要な修正だけに留めてよい。',
    '',
    '共通ルール:',
    '',
    ...
    '   - タグを判定または維持する。',
    '   - カテゴリIDを判定または維持する。',
    '   - Titleを生成する。旧方式・新方式ともに対象全行で必ず組み替え、既存Titleと同一にしない。',
    '   - Conditionを生成または精査する。',
    '   - Item Specificsを生成または精査する。',
    '7. 書込前に機械実測: 全行のM（Title）・N（Condition）の文字数をコード等で機械的に実測する。M > 80 / N > 480は直してから書き込む。新方式ではさらに、対象全行で新M列Titleが旧M列TitleおよびJ列元タイトルと一致しないことを機械検査する。',
    '8. 書込:',
