...
function getEbayMotorsTranslationSkillContent() {
  return [
    ...
    '  - **「どうしても推測した場合は warnings に明記すれば可」という抜け道は廃止**。推測値・創作値は、warnings に書く場合も含めて、いかなる場合もセルに書き込まない。色・寸法・キャラ名・カード数値・ブランド・付属品・限定/年代などを、ソース・画像・検索の裏付けなしに書くことを固く禁じる。',
    '- **タグ判定 (画像不要)**: タグ参照シート A 列の許可リストから **必ず 0 個または 1 個だけ**。配送カテゴリで選ぶ。リストにない新規タグ禁止。該当なしなら `[]`。部品用のタグは下記「タグ選択の目安」の対応表で選ぶ。対応表にあってもその回の許可リスト (TagShipping A 列) に無いタグ名は使わない (許可リストが常に優先)。「個別見積り対象」に該当する商品はタグを `[]` にし、warnings に `individual shipping quote required: <部品名>` を入れ、完了報告で必ず列挙する (翻訳・書込は通常どおり行う。保留にはしない)。',
    '- **画像の役割**: Title/IS/Description の正確性向上のため。タグ判定には使わない。',
    ...
    '',
    'タグは商品の「種類」で決める (寸法・重量の入力や計算はしない。箱の大きさはタグ側に織り込み済み)。左右セット・複数本は必ずセット側のタグにする。判断に迷うときは、より大きい (送料の高い) 方のタグを選ぶ。',
    '',
    '| タグ名 | 該当する商品の例 |',
    '|---|---|',
    '| 車部品 小物 | エンブレム、シフトノブ、スイッチ、キー、センサー、イグニッションコイル、ECU、リレー、小さな内装パーツ |',
    '| 車部品 中型 | ステアリングホイール、メーター、オルタネーター、スターター、ブレーキキャリパー片側、ウォーターポンプ、スロットルボディ、カーナビ本体、カーオーディオ、ETC 車載器 |',
