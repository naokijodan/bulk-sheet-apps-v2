...
function getEbayTranslationSkillContent() {
  return [
    ...
    '## 画像入力',
    '',
    '- 画像 URL (`=IMAGE("...")` から抽出) は **AI の vision 入力 (image_url) に直接渡す**。',
    '- ローカルダウンロード / curl / base64 化は禁止 (時間とリソースの無駄)。',
    '',
    '## 必要な接続',
    '',
    '- **MCP google-sheets** (各 AI 環境で接続済み前提)',
    '',
    '## ユーザーが渡すパラメータ',
    '',
    '- スプレッドシート ID',
    ...
    '| 列 | 内容 |',
    '|---|---|',
    '| B | プラットフォーム / C 商品 ID / D 価格 / E 日本語 title / F 日本語説明 / G 出品者 / H URL / I-R 画像 URL (=IMAGE) |',
    '',
    '## 書込先シート構造 (3 行目以降)',
    '',
    ...
    '## 動作手順 (最小)',
    '',
    '1. **取得**: タグ参照シート A 列 + ソースシート FORMULA mode で対象行を取得。画像 URL は正規表現 `=IMAGE\\("([^"]+)"` で抽出。',
    '2. **集約バッチ**: 行範囲を集約バッチサイズで区切り、各バッチで AI に 1 リクエスト (システム 1 回 + 商品 N 件 + 画像 URL を image_url 直渡し) → JSON 配列で受け取り。',
