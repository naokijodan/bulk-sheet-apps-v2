...
var SANITIZE_FIELDS_ = {
  ...
  'Power Tools': [
    'ブランド', 'タイプ', '電圧', 'バッテリーシリーズ', '製造国', '状態', '故障・不具合'
  ],
  'Laptops': [
...
var CATEGORY_RULES_ = {
  ...
  'Power Tools': {
    label: '電動工具',
    rules: [
      '- ブランド: マキタ→Makita / HiKOKI / リョービ→Ryobi / パナソニック→Panasonic 等。ソース記載を優先',
      '- タイプ: インパクトドライバー→Impact Driver / 丸ノコ→Circular Saw / ドリル→Drill 等',
      '- 電圧: ソース記載がある場合のみ（例: 18V / 36V / 100V）。推測で入力しない',
      '- バッテリーシリーズ: ソース記載がある場合のみ（例: Makita LXT 18V / HiKOKI Multi Volt）',
      '- 状態: 未使用→New / 中古→Used / ジャンク→For Parts Only',
      '- 故障・不具合: 動作不良/充電不可/異音等があれば記載。なければNA'
    ]
  },
  'Laptops': {
    label: 'ノートパソコン',
