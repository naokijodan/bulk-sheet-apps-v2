...
// Manga: Genre に基づく Age Level 動的判定
function matchMangaAgeLevel_(rowData) {
  if (!rowData) return '13+';
  var genre = String(rowData['Genre'] || '').toLowerCase();
  if (/adult|成人|18禁/.test(genre)) return '18+';
  if (/seinen|青年|josei|女性/.test(genre)) return '16+';
  return '13+';
}
