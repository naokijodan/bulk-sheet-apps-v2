...
<body>
    ...
    <form id="priceForm">
      ...
      <div class="section">
        ...
        <div class="form-group">
          <label>仕入れ値（円）</label>
          <input type="number" id="costYen" value="10000" min="0" step="1">
        </div>
        ...
        <div class="form-group" id="profitAmountGroup" style="display:none;">
          <label>利益額（円）</label>
          <input type="number" id="profitAmountYen" value="1500" min="0" step="100">
        </div>
      </div>
      ...
      <div style="text-align: center;">
        <button type="button" class="btn" onclick="calculatePrice()">計算実行</button>
  ...
  <script>
    ...
    function calculatePrice() {
      try {
        var payload = {
          costYen: parseFloat(document.getElementById('costYen').value),
        ...
        if (payload.profitMode === 'rate') {
          payload.profitRatePct = parseFloat(document.getElementById('profitRatePct').value);
        } else {
          payload.profitAmountYen = parseFloat(document.getElementById('profitAmountYen').value);
        }
    ...
    function showResult(result) {
      var html = '<h3>📊 計算結果</h3>';
      html += '<p><strong>販売価格:</strong> $' + result.priceUSD + '</p>';
      html += '<p><strong>関税込み価格:</strong> $' + result.taxIncludedPriceUSD + '</p>';
      html += '<p><strong>想定関税:</strong> $' + result.tariffUSD + '</p>';
      html += '<p><strong>送料:</strong> ¥' + result.shippingYen.toLocaleString() + '</p>';
      html += '<p><strong>利益:</strong> ¥' + result.profitYen.toLocaleString() + '</p>';
