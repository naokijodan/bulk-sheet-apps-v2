...
// DDU価格調整機能
  if (settings.dduAdjustmentEnabled) {
    ...
    if (priceMode !== 'TAX_INCLUDED') {
      ...
      var tagMap = null;
      try { tagMap = buildTagOverrideMap_(ss, settings); } catch (e) { tagMap = null; }
      var threshRef = String(settings.dduThreshold);
      if (settings.tagOverrideShippingCategory && tagMap) {
        threshRef = 'IFERROR(LET(___thr,INDEX(' + tsName + '!O:O,MATCH(D' + row + ',' + tsName + '!A:A,0)),IF(___thr="",' + settings.dduThreshold + ',___thr)),' + settings.dduThreshold + ')';
      }
      var dduFormula = '=IF(AND(R' + row + '>=' + threshRef + ', NOT(ISBLANK(S' + row + '))), S' + row + '-' + threshRef + ', "")';
      // Q→R（DDU販売価格）
      // R→S（DDP関税込み価格）
  ...
  try {
    ...
    if (settings.dduAdjustmentEnabled) {
      ...
      if (priceMode !== 'TAX_INCLUDED') {
        ...
        // タグ判定ON時の閾値参照テンプレート（TagShipping!O:O）。空セルは既定閾値にフォールバック
        var dduThreshRef = String(settings.dduThreshold);
        if (settings.tagOverrideShippingCategory && tagMap) {
          dduThreshRef = 'IFERROR(LET(___thr,INDEX(' + tsName + '!O:O,MATCH(D{row},' + tsName + '!A:A,0)),IF(___thr="",' + settings.dduThreshold + ',___thr)),' + settings.dduThreshold + ')';
        }
        var dduFormulas = [];
        for (var row = minRow; row <= maxRow; row++) {
          if (batchRowsSet[row]) {
            var rowDduThresh = dduThreshRef.replace(/\{row\}/g, String(row));
            dduFormulas.push([
              '=IF(AND(R' + row + '>=' + rowDduThresh + ', NOT(ISBLANK(S' + row + '))), S' + row + '-' + rowDduThresh + ', "")'
            ]);
          } else {
            dduFormulas.push(['']);
          }
