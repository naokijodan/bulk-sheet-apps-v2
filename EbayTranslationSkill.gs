...
function getEbayMotorsTranslationSkillContent() {
  return [
    ...
    '',
    'タグは商品の「種類」で決める (寸法・重量の入力や計算はしない。箱の大きさはタグ側に織り込み済み)。左右セット・複数本は必ずセット側のタグにする。判断に迷うときは、より大きい (送料の高い) 方のタグを選ぶ。**実際のタグ名は「基本名 + 半角スペース + 商品状態」** (例: `車部品 小物 中古` / `車部品 小物 新品`)。下表は基本名で書いてある。中古/新品の側は次で決める: ソースのタイトル・説明文に「新品」「未使用」「未開封」のいずれかが明記されているときだけ `新品` 側、それ以外 (記載なし・美品・中古・使用済み等) はすべて `中古` 側。これはタグ (送料・出品設定の軸) の決め方であり、Description の「転売ソースは全商品が中古前提・Pre-owned 基調」ルールとは独立に適用する (新品側のタグを選んでも Description は従来どおり Pre-owned 基調で書く)。その回の許可リスト (TagShipping A 列) に無い名前は使わない。ATV・UTV・船・除雪機・ゴルフカート等の部品は、大きさが同程度の「車部品 ○○」タグを使う (専用タグは作らない)。',
    '',
    '| タグ名 | 該当する商品の例 |',
    '|---|---|',
