...
function getEbayTranslationSkillContent() {
  ...
  return historyBlock.concat([
    ...
    '- **eBay カテゴリ ID 判定**: カテゴリ参照を翻訳前に 1 回 HTTP 取得する (`curl` / WebFetch): `https://naokijodan.github.io/bulksheet-ebay-categories/ebay-category-buckets.json` (`{ tagToGenre, buckets }` 形式、eBay US treeVersion 134 の公式確定 ID)。各商品で recommendedUserTags を決めたあと、その **タグ → `tagToGenre[タグ]` で genre** を得て、**`buckets[genre]` の候補から商品に最も合う categoryId を 1 つだけ**選ぶ。**候補リスト内の数値 ID のみ**採用し、リストにない番号を創作しない。該当 genre なし / 候補に合うものなし / 不明 / タグが空 の場合は categoryId も空。粒度は粗くてよい (詳細は Item Specifics が持つ)。',
    ...
    '3. **後処理**: 各 JSON の recommendedUserTags をタグ許可リストでフィルタ + 先頭 1 つだけ採用 (0 件は `[]`)。**categoryId** は上記「eBay カテゴリ ID 判定」に従い、取得した参照JSONの候補内 ID のみ採用 (無ければ空)。',
