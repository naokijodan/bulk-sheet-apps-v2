...
function getEbayTranslationFinalCheckContent() {
  ...
  return historyBlock.concat([
    ...
    '   ・Item Specifics の Type が Collectible 系か、対象年齢が 15+ か(幼児向け 3+ 等のままでないか)',
    '   ・コレクター扱い(Type が `Collectible 〜`＝Age Level を付与)の商品は、Title に `Collectible 15+` の文字列が入っているか(Age Level が 15+ で Title 表記は常に `Collectible 15+` 固定)。無ければ追加する(80字制限内)',
...
function buildEbayTranslationInstruction_(startRow, endRow, doGetUrl, doGetKey) {
  ...
  if (isMotorsMode) {
    ...
    extraPreFlightBullets = [
      ...
      '- 個別見積り対象の商品 (スキル本文「タグ選択の目安」) はタグを空欄にして通常どおり書き込み、完了報告で行番号と部品名を必ず1行ずつ列挙してください。'
    ];
  }
  ...
  return [
    ...
    '',
    '【着手前の約束（必読）】',
    '- 着手前に、あなたに登録されている本スキル（eBay 翻訳スキル。登録名・保存場所・形式はツールごとに異なってよく、ファイルとして探す必要はありません。あなたのツールでスキルとして読み込まれている内容を指します）を最初から最後まで全て読んでください。スキルは長いですが、守らなくてよいルール・読まなくてよい箇所・確認しなくてよい項目は一つもありません。特殊なルールも含め、書かれている全てを守る前提です。',
    '- スキルに書かれた手順・方式をそのまま実行してください。独自の段取り・自作スクリプト・別方式への置き換え、スキルに無い作業の追加は禁止です。',
    '- **【絶対・シート保護】このスプレッドシートで書き込んでよいのは、書込先シート (例: v5インポート) の「自分が担当した空き行」だけ。それ以外のシート (作業シート・TagShipping・設定・インポート用などのソースシートを含む、すべてのシート) は、値・数式・書式・シート構成を一切変更しない (読み取りのみ)。** ユーザーから「直して」「変えて」と頼まれた場合も同じ。このシートの式は表に出ていないライブラリ側のスクリプトが前提にしているため、見えている式だけで直すと壊れる。変更せず「このスキルではそのシートは変更できません」と報告して止まる。書込先シートでも、自分が書いた行以外 (既存行・見出し・式) は変更しない。',
    '- 書き込み前に、書込先シート「' + targetSheet + '」の H 列を実データで再取得し、空き行を正確に把握してください。行番号の推測や古い読み取り結果の流用は禁止です。既存データの上書きも、空き行の飛ばし（既存データの末尾から離れた行への書き込み）も絶対にしないでください。',
    '- 書き込み後は、書いた行が想定どおりの連続した行であること、既存データが一切変わっていないことを機械的に確認してください。',
...
function getEbayTranslationSkillContent() {
  ...
  var CHANGELOG = [
    { date: '2026-09-24', text: '①バーコード番号系 Item Specifics (UPC/EAN/JAN/ISBN/GTIN) は、画像・説明文で番号が読み取れる場合のみ書き、検査数字を計算して合わない番号は書かない (AI 創作の 13 桁が 4/4 件で検査数字不一致→eBay 登録エラー)。②タグ `売上` は最安値用でユーザー専用、AI は絶対に選ばない。該当タグが判断できない行は勝手に汎用タグで埋めず、その行を空欄にしてユーザーに 1 問ずつ確認する。ユーザー指示' },
    { date: '2026-09-20', text: 'シート保護ルールを追加 (書込先シートの担当行以外は、このスプレッドシートの全シートを値・数式・書式とも変更禁止。利用者側 AI が他シートの式を書き換えて壊す事故の再発防止)。ユーザー指示' },
    { date: '2026-09-04', text: 'ジーンズ・パンツ(11554/63863/11483/57989)は Size にウエストのインチ数値を使う例外を追加(公式サイズ標準化ガイドの標準ウエストサイズに従う。Regular で通る値: レディース 23〜33、メンズジーンズ 24〜32・34〜50 偶数、メンズパンツ 26〜32・34〜50 偶数。Waist Size 任意・メンズ Inseam 必須)。レディース裸の数字 18〜50 の Plus 専用はトップス・ワンピース等の米国サイズに限定。2026-09-04 Taxonomy API 実測' },
    { date: '2026-09-04', text: 'Country/Region of Manufacture に China を絶対使わないルールを追加 (中国ブランド・中国製でも Japan、Description の Made in China も禁止)。ユーザー指示' },
    { date: '2026-09-04', text: 'eBay 公式サイズ標準化ガイド(https://ebay.co.jp/knowledge/listing1/size-standardization-guide)と照合。S/M 等の複合表記を一部カテゴリで可としていた例外を撤回し全カテゴリで不可に、Taxonomy API の FREE_TEXT 表示でも自由入力不可を明記' },
  ...
  return historyBlock.concat([
    ...
    '- 完了後の報告は原則 **「完了。確認お願いします」の 1 行**。ただし、書込結果に **categoryId または タグが空欄の行、Item Specifics が 10 未満の行** があれば、その行番号と「なぜそうなったか」の理由を短く箇条書きで添える (例: `L58: categoryId 空 — タグに対応する候補が無いため` / `L60: IS 7件 — 画像不鮮明で確認できる仕様が少ないため`)。詳細な成功件数リストは不要。**例外: タグを判断できず D 列を空欄にした行がある場合は、1 行報告で終わらず、その行ごとに「行番号・商品・迷った候補タグ」を示してユーザーに 1 問ずつ確認する (回答が返るまで完了扱いにしない)。**',
    ...
    '  - **China は絶対に使わない (ユーザー固定ルール・2026-09-04)**: 中国ブランド (TOPTOY 等) や中国製と分かる商品でも `Country/Region of Manufacture` は Japan にする。Description にも `Made in China` と書かない。書込後 QA で `China` を機械検索して検出する',
    ...
    '- **バーコード番号 (UPC / EAN / JAN / ISBN / GTIN) の創作禁止 (絶対)**: これらの Item Specifics は、商品画像または日本語説明文に番号そのものが写っていて読み取れる場合だけ書く。ソースに番号が無ければ項目ごと書かない (空欄。`Does not apply` も書かない)。読み取れた場合も、書込前に検査数字 (EAN-13: 先頭から奇数桁×1・偶数桁×3 の合計を 10 の倍数に補う値／UPC-A 12 桁は先頭に 0 を補って 13 桁にし、同じ計算をする) をコードで計算し、合わない番号は読み取りミスとして書かない。10 項目目標のために「日本のバーコードらしい 13 桁」を作ることは、2026-09-24 に 4 件中 4 件が検査数字不一致で eBay 登録エラーになった実害があり、固く禁じる。',
    '- **タグ判定 (画像不要)**: タグ参照シート A 列の許可リストから **必ず 0 個または 1 個だけ**。配送カテゴリで選ぶ。リストにない新規タグ禁止。**タグ `売上` は禁止 (絶対)**: 最安値運用でユーザー自身が付ける専用タグであり、許可リストに載っていても AI は選ばない。「汎用」「その他」扱いで使うことも禁止。**該当タグが判断できない場合は、勝手に汎用的なタグで埋めない**。その行の D 列は空欄のまま書き込み、完了報告でその行番号と候補 (許可リスト内で迷ったタグ名) を挙げ、ユーザーに 1 問ずつ確認する。判断できないのに何かを埋めることが最も重い違反。日本作品のキャラクターグッズ (バッグ、ポーチ、スカーフ、文具、缶バッジ、ポスター、シール、布パネル等) は、物理形状が衣類・バッグ系でも、配送タグとして適切なら `アニメグッズ` を優先する。ヘアピン等、許可リストに専用タグがあり配送実態が近いものだけ `ヘアアクセサリー` 等を使う。カードの配送タグは 1 枚のみ=単品系 (未鑑定/PSA 等)、2 枚以上のセット・まとめ=まとめ売り (カテゴリの Mixed Lots と枚数判定を連動させる)。',
    ...
    '  - **【必須】Title に `Collectible 15+` を必ず入れる**: コレクター扱い (Item Specifics の Type を `Collectible 〜` にする＝Age Level を付与する) にする商品 (アニメグッズ・キャラ/アイドルグッズ・フィギュア・ドール等) は、Title (M 列) に必ず `Collectible 15+` の文字列を入れる。Age Level は `15+` に統一するため Title 表記も常に `Collectible 15+` 固定 (Age Level 欄と一致させない)。Title 80 字制限内に必ず収める。 なお、トレカ／スポーツトレカ等のカード商品 (上記カテゴリ 4 ID 該当品) も、Age Level の付与有無に関わらず Title に `Collectible 13+` を必ず入れる。',
    '  - **【アニメグッズ・キャラ/アイドルグッズは年齢記載が無くてもコレクター扱い (重要)】**: アクリルスタンド (アクスタ) / アクリルキーホルダー / 缶バッジ / ブロマイド / ポストカード / タオル・バッグ等のファングッズや、実在アイドル・バンドの音楽メモラビリアは、元データに年齢の記載が一切無くても **必ずコレクター向けアイテムとして扱う**。具体的には (1) `categoryId` は `アニメ` bucket から付与 (キーホルダー→261057 / 缶バッジ→261061 / ポスター・タペストリー→261063、一意の細分類が無い物は 69528)、(2) Item Specifics の `Type` を `Collectible 〜` にする (例 `Collectible Acrylic Stand` / `Collectible Acrylic Keychain`)、(3) Item Specifics に `Age Level` を **必ず 1 つ付与する (既定 `15+`)**。空欄にしない。categoryId を空欄にしない。',
    '  - **Age Level / Recommended Age Range / Age** は元データに `3+` `6+` `8+` 等の幼児向け年齢が書かれていても **必ず `15+` にする (フィギュア・アニメグッズ等のコレクター扱いは一律 `15+`)**。元データの幼児向け年齢は無視',
    ...
    '  - **決め事2: Size は文字サイズ `S / M / L / XL` (2XS〜2XL) だけ使う**。タグが数字でも実寸から文字に読み替える。cm・Free・裸の数字は使わない。文字サイズ＋Regular は服の主要カテゴリ全てで通る。理由: トップス・ワンピース等の米国サイズでは、レディースの裸の数字 `18〜50` が Plus 専用 (`40` を Regular と組むとエラー)、メンズでは `52` 以上が Big & Tall 専用 (ジーンズ・パンツのウエスト数値は末尾の例外を参照)。欧州ブランドのタグ数字を残す場合だけ `EU 40` / `IT 40` の国別表記にする (Regular で可)。`One Size` は帽子・小物用で、服では Tシャツ・トップス等は可だがスーツ 3001・ドレスシャツ 57991・バッグは不可。`S/M` `M/L` 等の複合表記 (1つの Size 欄に複数サイズ) は全カテゴリで使わない (eBay 公式のサイズ標準化ガイドで「S/M/L など複数の Size を組み合わせた値」は非サポート。2026-09-01 の API 実測で一部カテゴリが通っていたが、ガイドに従い使わない)。実寸 cm は Title・Description に残す (情報は失われない)。**例外: ジーンズ・パンツ (レディース 11554/63863、メンズ 11483/57989) だけは、ウエストのインチ数値を Size にする** (eBay 公式サイズ標準化ガイドの「Jeans and pants = 標準ウエストサイズ」に従う。タグの W30 → `30`、実寸 cm は ÷2.54 して最も近い許容値に丸める)。Regular で通る値 (2026-09-04 Taxonomy API 実測): レディース `23〜33` (1刻み)、メンズジーンズ `24〜32` (1刻み) と `34〜50` (偶数)、メンズパンツ `26〜32` (1刻み) と `34〜50` (偶数)。範囲外 (レディース 34 以上・メンズ 52 以上など) は文字サイズに読み替える。あわせて `Waist Size` (任意) に `30 in` 形式、メンズは必須の `Inseam` に `32 in` 形式で入れ、実寸は Title・Description にも残す。',
    ...
    '  - 参照ページに無いカテゴリで Size を求められた場合は、リスト外の値を推測で入れず、公式許容値 (Taxonomy API getItemAspectsForCategory) を確認してから入れる。Taxonomy API で Size が `FREE_TEXT` と表示されていても自由入力は不可 (公式ガイド明記)。API の getItemAspectsForCategory が返す値、または出品画面のドロップダウン値だけを使う。',
    ...
    '6. **完了報告**: 「完了。確認お願いします」と表示して終了。ただし categoryId / タグが空欄の行や Item Specifics が 10 未満の行があれば、その行と理由を短く添える。タグを判断できなかった行は、候補を示してユーザーに 1 問ずつ確認し、回答を得てから D 列を埋める (勝手に埋めない)。',
    '',
    '## 実行運用 (速度・信頼性)',
    '',
...
function getRelistingTranslationSkillContent() {
  // ★スキル本文を変えたら CHANGELOG の先頭に {date, text} を1件追加(新しい順・事実を正確に)。
  var CHANGELOG = [
    { date: '2026-07-02', text: 'コレクター扱い(フィギュア/アニメ系)の再作成 Title と Age Level を 15+ に統一(Collectible 15+)。カード商品は Collectible 13+ のまま' },
  ...
  return historyBlock.concat([
    ...
    '- 玩具・キャラクター系はCollectiblesカテゴリ優先。Age Levelは元データに幼児向け年齢が書かれていても必ず `15+` にする（コレクター扱いは一律 `15+`）。',
    '- **【必須】コレクター扱いの商品は再作成 Title に `Collectible 15+` を必ず入れる**: コレクター扱い (Item Specifics の Type を `Collectible 〜` にする＝Age Level を付与する) にする商品 (アニメグッズ・キャラ/アイドルグッズ・フィギュア・ドール等) は、再作成する Title (M 列) に必ず `Collectible 15+` の文字列を入れる。Age Level は `15+` に統一するため Title 表記も常に `Collectible 15+` 固定 (Age Level 欄と一致させない)。Title 80 字制限内に必ず収める。 なお、トレカ／スポーツトレカ等のカード商品 (上記カテゴリ 4 ID 該当品) も、Age Level の付与有無に関わらず Title に `Collectible 13+` を必ず入れる。',
    '',
    ...
    '| ブランド国が判定できる場合 | その国 |',
    '| 辞書外で不明な場合 | Japan |',
    '| 中国ブランド・中国製 (China は絶対に使わない。Description の Made in China も禁止) | Japan |',
    '',
    '### eBayカテゴリID（F列）',
    '',
...
function getEbayMotorsTranslationSkillContent() {
  return [
    ...
    'description: Convert Japanese marketplace product rows (Mercari, Yahoo Auctions, Hard Off, Mercari Shops) for eBay Motors US parts and accessories (car, motorcycle, ATV/UTV, commercial truck, boat, RV, snowmobile, golf cart parts) into English eBay listing import data and write them to Google Sheets: Title, Condition/Description, Item Specifics from the official per-category aspect file, tag, and the eBay Motors categoryId. Use when the user asks to run "ebay-motors-translation", translate vehicle parts for eBay Motors, or fill the bulk sheet for Motors rows. It produces nothing else and creates no compatibility (fitment) data.',
    '---',
    '',
    '<!-- 【変更履歴】人向けメモ。スキル実行には影響しません（新規の方は読み飛ばして OK ／ 更新の方はここで変更点を確認）。',
    '- 2026-09-25 (5件目): タグは当面「常に中古側」に固定 (新品側タグはユーザーが解禁するまで使わない)。ユーザー指示',
    '- 2026-09-25 (4件目): タグの中古/新品の決め方を明文化 (ソースに「新品」「未使用」「未開封」の明記があるときだけ新品側、それ以外は中古側。Description の Pre-owned 基調ルールとは独立)。タグ2件追加 (車 電装品／車 アクセサリー 小物)。ユーザー指示',
    '- 2026-09-25 (3件目): タグ名を「基本名＋商品状態」に変更 (例: 車部品 小物 中古 / 車部品 小物 新品、TagShipping に中古25件・新品25件を作成)。タグ4件追加 (車部品 マット・カバー／車 マニュアル・カタログ／バイク用品 アパレル／バイク用品 小物)。ATV・船・除雪機等の部品は大きさで車部品タグを使う旨を追記。ユーザー指示',
    '- 2026-09-25 (2件目): 本番スキル 2026-09-24 の追加ルールを反映。①バーコード番号系 Item Specifics (UPC/EAN/JAN/ISBN/GTIN) は読み取れた番号のみ・検査数字が合わないものは書かない。②タグ `売上` はユーザー専用で AI は絶対に選ばない。判断できない行は汎用タグで埋めず空欄にして完了報告で 1 問ずつ確認する。ユーザー指示',
    '- 2026-09-25: 部品用タグ一覧を確定 (TagShipping に21タグ作成済み)。「タグ選択の目安」節と「個別見積り対象 (送料タグ無し・完了報告で必ずアラート)」ルールを追加。未決事項「部品用タグリスト」を解決済みに変更。ユーザー指示',
    '- 2026-09-20: シート保護ルールを追加 (書込先シートの担当行以外は、このスプレッドシートの全シートを値・数式・書式とも変更禁止。利用者側 AI が他シートの式を書き換えて壊す事故の再発防止)。ユーザー指示',
    ...
    '',
    'バージョン: 2026-09-25 (base: ebay-translation 2026-09-04)',
    '',
    ...
    '- **全工程を確認なしで一気通貫で実行**。途中で「これを実行してよいか」「次に進むか」は聞かない。',
    '- **途中承認は全てスキップ**。画像取得・カテゴリ参照取得・シート読込・シート書込・QA修正など、このスキル内で必要な通常操作はユーザー確認を挟まず実行する。実行環境が表示する権限ダイアログ等、AI側で省略できないシステム承認だけは例外。',
    '- 完了後の報告は原則 **「完了。確認お願いします」の 1 行**。ただし、書込結果に **categoryId または タグが空欄の行、Item Specifics が 10 未満の行** があれば、その行番号と「なぜそうなったか」の理由を短く箇条書きで添える (例: `L58: categoryId 空 — モータースのカテゴリ参照に合致する末端カテゴリが無いため` / `L60: IS 7件 — 画像不鮮明で確認できる仕様が少ないため`)。詳細な成功件数リストは不要。 **個別見積り対象の行 (下記「タグ選択の目安」参照) は、タグ空欄の理由として必ず `L58: 個別見積り対象 (バンパー) — 送料タグなし、送料は手動で決定` の形で1行ずつ列挙する (省略禁止)。**',
    ...
    '  - **「どうしても推測した場合は warnings に明記すれば可」という抜け道は廃止**。推測値・創作値は、warnings に書く場合も含めて、いかなる場合もセルに書き込まない。色・寸法・キャラ名・カード数値・ブランド・付属品・限定/年代などを、ソース・画像・検索の裏付けなしに書くことを固く禁じる。',
    '- **バーコード番号 (UPC / EAN / JAN / ISBN / GTIN) の創作禁止 (絶対)**: これらの Item Specifics は、商品画像または日本語説明文に番号そのものが写っていて読み取れる場合だけ書く。ソースに番号が無ければ項目ごと書かない (空欄。`Does not apply` も書かない)。読み取れた場合も、書込前に検査数字 (EAN-13: 先頭から奇数桁×1・偶数桁×3 の合計を 10 の倍数に補う値／UPC-A 12 桁は先頭に 0 を補って 13 桁にし、同じ計算をする) をコードで計算し、合わない番号は読み取りミスとして書かない。10 項目目標のために「日本のバーコードらしい 13 桁」を作ることは、2026-09-24 に 4 件中 4 件が検査数字不一致で eBay 登録エラーになった実害があり、固く禁じる。',
    '- **タグ判定 (画像不要)**: タグ参照シート A 列の許可リストから **必ず 0 個または 1 個だけ**。配送カテゴリで選ぶ。リストにない新規タグ禁止。該当なしなら `[]`。**タグ `売上` は禁止 (絶対)**: 最安値運用でユーザー自身が付ける専用タグであり、許可リストに載っていても AI は選ばない。「汎用」「その他」扱いで使うことも禁止。**該当タグが判断できない場合は、勝手に汎用的なタグで埋めない**。その行の D 列は空欄のまま書き込み、完了報告でその行番号と候補 (許可リスト内で迷ったタグ名) を挙げ、ユーザーに 1 問ずつ確認する。判断できないのに何かを埋めることが最も重い違反。 部品用のタグは下記「タグ選択の目安」の対応表で選ぶ。対応表にあってもその回の許可リスト (TagShipping A 列) に無いタグ名は使わない (許可リストが常に優先)。「個別見積り対象」に該当する商品はタグを `[]` にし、warnings に `individual shipping quote required: <部品名>` を入れ、完了報告で必ず列挙する (翻訳・書込は通常どおり行う。保留にはしない)。',
    '- **画像の役割**: Title/IS/Description の正確性向上のため。タグ判定には使わない。',
    ...
    '',
    '## タグ選択の目安 (D 列・部品の種類で選ぶ)',
    '',
    'タグは商品の「種類」で決める (寸法・重量の入力や計算はしない。箱の大きさはタグ側に織り込み済み)。左右セット・複数本は必ずセット側のタグにする。判断に迷うときは、より大きい (送料の高い) 方のタグを選ぶ。**実際のタグ名は「基本名 + 半角スペース + 商品状態」** (例: `車部品 小物 中古` / `車部品 小物 新品`)。下表は基本名で書いてある。**中古/新品の側: 当面は常に `中古` 側を選ぶ (2026-09-25 ユーザー指示)。ソースに「新品」「未使用」「未開封」とあっても `中古` 側にする。`新品` 側のタグは TagShipping に存在するが、ユーザーが明示的に「新品タグを使う」と指示するまで選ばない。** (将来解禁されたときの決め方: ソースのタイトル・説明文に「新品」「未使用」「未開封」のいずれかが明記されているときだけ `新品` 側、それ以外はすべて `中古` 側。タグは送料・出品設定の軸であり、Description の「転売ソースは全商品が中古前提・Pre-owned 基調」ルールとは独立。)その回の許可リスト (TagShipping A 列) に無い名前は使わない。ATV・UTV・船・除雪機・ゴルフカート等の部品は、大きさが同程度の「車部品 ○○」タグを使う (専用タグは作らない)。',
    '',
    '| タグ名 | 該当する商品の例 |',
    '|---|---|',
    '| 車部品 小物 | エンブレム、シフトノブ、スイッチ、キー、センサー、イグニッションコイル、ECU、リレー、小さな内装パーツ |',
    '| 車部品 中型 | ステアリングホイール、メーター、オルタネーター、スターター、ブレーキキャリパー片側、ウォーターポンプ、スロットルボディ |',
    '| 車部品 テールランプ 単品 | テールランプ片側、フォグランプ、ウインカー、サイドマーカー |',
    '| 車部品 テールランプ 左右セット | テールランプ左右セット |',
    '| 車部品 ヘッドライト 単品 | ヘッドライト片側 |',
    '| 車部品 ヘッドライト 左右セット | ヘッドライト左右セット |',
    '| 車部品 ミラー 片側 | ドアミラー片側、ルームミラー |',
    '| 車部品 ミラー 左右セット | ドアミラー左右セット |',
    '| 車部品 モール・長物 小 | モール、ワイパーアーム、ドアハンドル、細いトリム (長さ 1m 程度まで) |',
    '| 車部品 グリル・パネル 中 | フロントグリル、フェンダーライナー、小さいパネル、小型スポイラー |',
    '| 車部品 ラジエター・コンデンサー | ラジエター、A/C コンデンサー、インタークーラー |',
    '| 車部品 足回り 単品 | ショックアブソーバー1本、コントロールアーム、ドライブシャフト、ナックル |',
    '| 車部品 足回り セット | ショック4本セット、サスペンションキット |',
    '| 車部品 重量小物 | ブレーキローター、フライホイール、ハブ、プーリー (小さいが重いもの) |',
    '| 車部品 エンジン補機 大 | ターボチャージャー、インテークマニホールド、エキゾーストマニホールド、A/C コンプレッサー、ミッション部品 |',
    '| 車部品 ホイール 1本 タイヤなし | ホイール1本 (タイヤ無し、18 インチ程度まで) |',
    '| 車部品 ホイール 1本 タイヤ付 | ホイール1本 (タイヤ付き) |',
    '| バイク部品 小物 | レバー、グリップ、ミラー、ウインカー、キャブレター部品、スイッチ |',
    '| バイク部品 中 | キャブレター本体、ヘッドライト、メーター、シート、ステップ、スプロケット |',
    '| バイク部品 マフラー 単品 | サイレンサー、エキゾーストパイプ (長さ 1m 程度まで) |',
    '| バイク部品 大 | 燃料タンク、ホイール、フロントフォーク、スイングアーム |',
    '| 車部品 マット・カバー | フロアマット、シートカバー、ボディカバー、サンシェード (軽いが箱が大きい) |',
    '| 車 マニュアル・カタログ | 整備書、パーツカタログ、取扱説明書、カタログ類 |',
    '| バイク用品 アパレル | ジャケット、パンツ、レインウェア (ヘルメットは除外のまま) |',
    '| バイク用品 小物 | グローブ、ブーツ、プロテクター、バッグ |',
    '| 車 電装品 | カーナビ本体、カーオーディオ、ETC 車載器、ドライブレコーダー、レーダー探知機、スピーカー |',
    '| 車 アクセサリー 小物 | スマホホルダー、カップホルダー、芳香剤、LED バルブ、ワイパーゴム、キーケース等の小さなアクセサリー |',
    '',
    '**個別見積り対象 (タグを付けない)**: バンパー、ドア、ボンネット、フェンダー、シート (車)、ホイール2本以上、車のマフラー (フルシステム・長さ 1m 超)、バイクのカウルセット、エンジン本体・ミッション本体。料金表の上限 (30kg) を超えるか箱が 1.5m 級になるため、固定の送料タグでは損失になる。該当する商品は翻訳・書込を通常どおり行い、タグは `[]`、warnings に `individual shipping quote required: <部品名>` を記録し、完了報告で必ず 1 行ずつ列挙する (保留にはしない)。',
    '',
    ...
    '- **部品用タグリスト**: 解決済み (2026-09-25)。TagShipping に21タグを作成し、「タグ選択の目安」に対応表を記載。',
    ''
  ].join('\n');
}
