...
### 同日追加: 商品状態の判断ルール2項目＋既存行修正（commit 6be6855、BulkToolsLib clasp push済み・HEAD一致を clasp pull で実測）
- 背景（Fact、LINE サポートログ `~/Desktop/[LINE]秋場史康さんRoyalサポート（ebay）.txt` 2026-09-03/09-09）: 利用者の中古時計が新品として出品され、バイヤーから「中古なのに新品表示」「新品未使用・箱と保証付きのはずが無い」とクレーム。利用者の TagShipping には「時計 中古」のような状態別タグがある。
- 仕組み（Fact、コード確認）: V5作業シートのタグ判定ONでは AE列(状態) = TagShipping P列をD列タグで INDEX/MATCH（PがAI/空なら$P$2）。つまり新品/中古はタグ経由で決まる。ソースシートに状態専用列は無い（説明文と画像から読む）。スキル本文は「タグは配送カテゴリで選ぶ・画像不使用」だったため、状態別タグがあると状態を見ずに選ぶ構造だった（Inference）。
- 変更（指示文のみ）: 着手前の約束に ⑥状態（新品/中古/傷・汚れ・欠品）はソース原文と画像を見て1行ずつ判断、中古を新品扱いしない（新品=箱・保証書付きと解釈される） ⑦状態で分かれたタグは状態判断に基づいて選ぶ、を追加。既存行「タグ判定 (画像は使わない)」を「基本は配送カテゴリで選ぶ。状態で分かれたタグがある場合のみ、上記の状態判断を使う」に修正。
- 2者レビュー PASS。レビュワー指摘: スキル本文側（getEbayTranslationSkillContent）に「タグ判定は配送カテゴリ／画像不要／画像はタグ判定に使わない」が4箇所（実行運用ステップ3・タグ判定・画像の役割・タグ(D列)セクション）残っており指示文と食い違う。**次セッションでスキル本文側を同期する（CHANGELOG 追加・版更新・SKILL.md 2箇所も同期）。**
- 未確認（Unknown）: 利用者側の TagShipping P列/P2 の実設定。9/3 の件が P列設定起因か AI のタグ誤選択起因かは未特定。
...
### 未実施
- なし（GitHub push 済み: 9696adc / 998f6d1 / 5cee073 / bec5e9c）。翌日以降の実機確認（自動移行ログ→翌々日 C2 当日値）のみ。
