...
### 1. ユーザーに確認（必須）
前回のセッションとの間に、ユーザーが**大きな修繕を1つ行っている**。
まずユーザーに以下を確認すること：
- **「前回のセッション後に行った修繕の内容を教えてください」**
- その修繕の影響範囲を把握してから作業を開始する
- 修繕内容によっては、HANDOVER.mdの「次にやること」の優先順位が変わる可能性がある

### 2. 準備（ルール・ノート確認）
- `~/.claude/CLAUDE.md` + `~/.claude/rules/` を読む
- `~/Desktop/開発ログ/` の関連ノートを読む
- `git log --oneline -10` + `git status` で現状把握
- ユーザーの修繕内容を反映した上で、作業方針をユーザーに提示して承認を得る
